...
class DrivingCorridor
{
  private:
    ...
    IPoint2 GetPointInside(Polygon polygon);

    Point_range PolygonToShape(Polygon polygon);

    Point_range TransformSourceToTarget(Point_range coordinates) const;

    Point_range TransformTargetToSource(Point_range coordinates) const;
  ...
  public:
    ...
    DrivingCorridor(Polygon lane_polygon, Polygon transformed_polygon, std::list<Polygon> dc_lanes, float max_triangulation_edge_lenth = 1);

    ~DrivingCorridor();

    Polyhedron TransformOriginalToMapped(Polyhedron &input_polyhedron) const;

    Polyhedron TransformMappedToOriginal(Polyhedron &input_polyhedron) const;
};

...
/// extruded polygon.
template <class HDS> class ExtrudeZ : public CGAL::Modifier_base<HDS>
{
  public:
    Polygon polygon;
    std::pair<float, float> bounds;

    ExtrudeZ(Polygon poly, std::pair<float, float> bnds) : polygon(poly), bounds(bnds)
    {
    }
...
template <class HDS> class ExtrudeY : public CGAL::Modifier_base<HDS>
{
  public:
    ...
    ExtrudeY(Polygon poly, std::pair<float, float> bnds) : polygon(poly), bounds(bnds)
    {
    }
    ...
    void operator()(HDS &hds)
    {
        ...
        for (auto it = polygon.vertices_begin(); it != polygon.vertices_end(); ++it)
        {
            B.add_vertex(Point3(it->x(), bounds.first, it->y()));
            B.add_vertex(Point3(it->x(), bounds.second, it->y()));
        }
